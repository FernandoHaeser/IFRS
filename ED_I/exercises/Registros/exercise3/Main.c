...
bool situacaoAluno(int index)
{
    float media = mediaAluno(index);
    int faltas = listaAlunos[index].faltas;
    bool aprovado = (media >= 7 && faltas < 10) ? true : false;
    return aprovado;
}

void lerInfosAluno(int index)
{
    ...
    printf("Situação Aluno: %s\n", (situacaoAluno(index)) ? aprovado : reprovado);
...
void editarEstudante(int index)
{
...
int main()
{
    ...
    while (keep)
    {
        ...
        case 1:
            ...
            scanf(" %d", &choice);
            lerInfosAluno(choice - 1);
            break;
        case 2:
            printf("\n\nQual estudante deseja editar (1,2,3...)? ");
            scanf(" %d", &choice);
            editarEstudante(choice - 1);
            break;
        case 3:
            printf("\n\nAdiossss...");
            return 0;
